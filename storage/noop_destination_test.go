...
// State. State() must not return nil, or MultiSnapshot.Overview() will
// nil-dereference panic when collected by MultiDestination.State().
...
func TestMultiDestination_WithNoopDestination_State(t *testing.T) {
	...
	// Should not panic - this is the core regression test for nil State
	_ = state.Overview()
	_ = state.Amount()
	_ = state.Title()
	_ = state.Concurrency()
