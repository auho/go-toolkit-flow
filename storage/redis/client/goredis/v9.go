...
func (v *V9) HashScan(ctx context.Context, keyName string, cursor uint64, match string, count int64) ([]string, uint64, error) {
	return v.Client.HScan(ctx, keyName, cursor, match, count).Result()
}

// HashMSet batch-sets hash fields using a pipeline.
func (v *V9) HashMSet(ctx context.Context, keyName string, entries []map[string]any) error {
...
func (v *V9) ListPush(ctx context.Context, keyName string, entries []string) error {
...
func (v *V9) SetScan(ctx context.Context, keyName string, cursor uint64, match string, count int64) ([]string, uint64, error) {
	return v.Client.SScan(ctx, keyName, cursor, match, count).Result()
}

// SetAdd adds members to a set.
func (v *V9) SetAdd(ctx context.Context, keyName string, entries []string) error {
	anyEntries := make([]any, 0, len(entries))
	for _, e := range entries {
		anyEntries = append(anyEntries, e)
	}
...
func (v *V9) SortedSetScan(ctx context.Context, keyName string, cursor uint64, match string, count int64) ([]string, uint64, error) {
...
func (v *V9) SortedSetAdd(ctx context.Context, keyName string, entries []map[any]float64) error {
	pipe := v.Client.Pipeline()
	for _, entry := range entries {
